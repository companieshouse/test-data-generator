...
- POST: Sending a POST request to create UVID for a user `{Base URL}/test-data/identity/identityUvid` will generate a new UVID for a user. The request body must include `Identityspec` parameter to used to validate user identity.
    - `email`: The email id of the user. This is mandatory.
    - `user_id`: The user id of the user. This is mandatory.
    - `verification_source`: The verification source of the user. This is mandatory.
    
    A usage example looks like this: `{ "email": "test@test.com", "user_id": "userid", "verification_source": "TEST" }`
- DELETE: USes the same endpoint and data as the identity. Deleting the identity deletes all associated identityUvid data.
