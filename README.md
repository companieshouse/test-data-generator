...
- POST: Sending a POST request to `{Base URL}/test-data/company` will generate a new test company and accompanying Authcode. The request body can include an optional `CompanySpec` parameter to customise the generated company.
  ...
  - `has_uk_establishment`: Boolean value to determine if the oversea company has a UK establishment. Defaults to false, `true` value will create an oversea company with a UK establishment. Used alongside `oversea-company` company type. **Note:** This field is maintained for backward compatibility. Use `number_of_uk_establishments` for specifying multiple establishments.
  ...
  - `accounts_due_status`: Set the accounts and confirmation statement due dates of the company by providing accounts_due_status (e.g., `overdue`, `due-soon`). Defaults to current date. `limited-partnership` companies do not file accounts, so no accounts details are created for them.
  ...
  - A usage example for creating `oversea-company` with `has_uk_establishment`  looks like this: `{"type": "oversea-company", "has_uk_establishment": true}`, this will create an oversea company with hardcoded values and 1 UK establishment
  - A usage example for creating `oversea-company` with `number_of_uk_establishments` looks like this: `{"type": "oversea-company", "number_of_uk_establishments": 3}`, this will create an oversea company with 3 UK establishments
  - A usage example for creating `oversea-company` with no UK establishments: `{"type": "oversea-company", "number_of_uk_establishments": 0}`, this will create an oversea company without any UK establishments
  ...
  - A usage example for creating a company with psc: `{ "number_of_pscs": 2, "psc_type": ["legal", "individual"] }`
  ...
  - A usage example of creating a company with an inactive psc: `{ "number_of_pscs": 1, "psc_active": false }`
  - A usage example for creating a company with registered office in dispute: `{ "registered_office_is_in_dispute": true }`
  - A usage example for creating a company with undeliverable office address: { "undeliverable_registered_office_address": true}
...
#### Creating, Retrieving, Updating and Deleting Company Exemptions
- POST: Sending a POST request to `{Base URL}/test-data/internal/exemptions` will create or update a company exemptions entry in the `company_exemptions` db collection. The exemption document is auto-generated based on the type provided.
  ...
  - `exemption_type`: The type of exemption to generate. Optional — if omitted, a random type is selected from the allowed list below. Must be one of the allowed types, otherwise a validation error is returned.
  - `data`: Optional custom exemption data. If omitted, data is auto-generated.

  **Allowed Exemption Types:**
  ...
  **Error Responses:**
  - Invalid exemption type: Returns 400 Bad Request with error message listing allowed types
  - Missing company_number: Returns 400 Bad Request

- GET: Sending a GET request to `{Base URL}/test-data/internal/exemptions/{companyNumber}` will retrieve the company exemptions entry by company number. Returns 200 OK on success, 404 Not Found if the exemption does not exist.

- PUT: Sending a PUT request to `{Base URL}/test-data/internal/exemptions/{companyNumber}` will update an existing company exemptions entry. The exemption must exist, and the created timestamp is preserved while the updated timestamp is set to now. Supports the same exemption_type validation as POST. Returns 200 OK on success, 404 Not Found if exemption does not exist, 400 Bad Request for invalid exemption type.

- DELETE: Sending a DELETE request on the endpoint `{Base URL}/test-data/internal/exemptions/{companyNumber}` will delete the company exemptions entry. Returns 204 No Content on success, 404 Not Found if exemption does not exist.
